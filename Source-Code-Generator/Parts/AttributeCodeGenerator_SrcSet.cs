namespace SourceCodeGenerator.Parts
{
    public partial class AttributeCodeGenerator
    {
        ...
        private string CodeForSrcSetAttribute(TagCodeGenerator tag) =>
            !IsSrcSetAttribute
                ...
                : $@"
    /// <summary>
    /// Add another name/number pair to the {Key} attribute on the &lt;{tag.TagName}&gt; tag {CommentForPreprocessing}
    /// </summary>
    ...
    {Method(tag.ClassName)}(string name, int multiplier) => this.Attr(""{Key}"", UriHelpers.UriEncode(name) + "" "" + multiplier + (multiplier > 8 ? ""w"" : ""x""){GetSeparator()});";
